...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var host = document.getElementById('ts-podcast-3d');
  if (!host || !window.THREE) return;
  ...
  var cBois = document.createElement('canvas');
  cBois.width = 512; cBois.height = 512;
  (function (g, w, h) {
    g.fillStyle = BOIS; g.fillRect(0, 0, w, h);
    ...
    for (var i = 0; i < 900; i++) {
      var yy = Math.random() * h, l = 40 + Math.random() * 200;
      g.strokeStyle = Math.random() > 0.5 ? 'rgba(0,0,0,0.09)' : 'rgba(255,205,150,0.09)';
      g.lineWidth = 0.6 + Math.random() * 1.8;
      g.beginPath(); g.moveTo(Math.random() * w, yy);
      g.lineTo(Math.random() * w + l, yy + (Math.random() - 0.5) * 5); g.stroke();
    }
    g.globalCompositeOperation = 'destination-out';
    var deg = g.createLinearGradient(0, 0, 0, h * 0.62);
    deg.addColorStop(0, 'rgba(0,0,0,1)');
    deg.addColorStop(1, 'rgba(0,0,0,0)');
    g.fillStyle = deg; g.fillRect(0, 0, w, h * 0.62);
    g.globalCompositeOperation = 'source-over';
  })(cBois.getContext('2d'), 512, 512);

  var texBois = new THREE.CanvasTexture(cBois);
  var bureau = new THREE.Mesh(
    new THREE.PlaneGeometry(15, 10),
    new THREE.MeshStandardMaterial({ map: texBois, transparent: true, roughness: 0.5, metalness: 0.05 })
  );
  bureau.rotation.x = -Math.PI / 2;
  bureau.position.set(0, 0, -1);
  bureau.receiveShadow = true;
  plateau.add(bureau);

  var mAlu = new THREE.MeshStandardMaterial({ color: 0xb9bcc2, roughness: 0.3, metalness: 0.88 });
  var mSombre = new THREE.MeshStandardMaterial({ color: 0x1d1f25, roughness: 0.6, metalness: 0.3 });
  var mGrille = new THREE.MeshStandardMaterial({ color: 0x33363e, roughness: 0.85, metalness: 0.25 });
  var mCaoutchouc = new THREE.MeshStandardMaterial({ color: 0x15161a, roughness: 0.95, metalness: 0 });
  var mAccent = new THREE.MeshStandardMaterial({
    color: ACCENT, roughness: 0.3, metalness: 0.7, emissive: ACCENT, emissiveIntensity: 0.2
  });

  var cEcran = document.createElement('canvas');
  cEcran.width = 512; cEcran.height = 320;
  var gE = cEcran.getContext('2d');
  var texEcran = new THREE.CanvasTexture(cEcran);

  function barre(i, piste) {
    var s = Math.sin(i * 12.9898 + piste * 78.233) * 43758.5453;
    var r = s - Math.floor(s);
    var env = 0.35 + 0.65 * Math.abs(Math.sin(i * 0.06 + piste * 1.7));
    return (0.18 + r * 0.82) * env;
  }

  function dessineEcran(t) {
    var w = 512, h = 320;
    gE.fillStyle = '#0f1117'; gE.fillRect(0, 0, w, h);

    gE.fillStyle = '#191c25'; gE.fillRect(0, 0, w, 30);
    var pastilles = ['#ff5f57', '#febc2e', '#28c840'];
    for (var i = 0; i < 3; i++) {
      gE.fillStyle = pastilles[i];
      gE.beginPath(); gE.arc(20 + i * 18, 15, 5.5, 0, 6.283); gE.fill();
    }
    gE.fillStyle = '#767b88';
    gE.font = '600 14px Helvetica, Arial, sans-serif';
    gE.textAlign = 'center';
    gE.fillText(TITRE_ECRAN, w / 2, 20);

    var defile = t * 26;
    [0, 1].forEach(function (piste) {
      var y0 = 66 + piste * 122, ht = 96;
      gE.fillStyle = '#151821';
      gE.fillRect(14, y0, w - 28, ht);
      gE.fillStyle = piste === 0 ? ACCENT : ACCENT2;
      var centre = y0 + ht / 2;
      for (var x = 0; x < w - 34; x += 5) {
        var idx = Math.floor((x + defile) / 5);
        var a = barre(idx, piste) * (ht / 2 - 6);
        gE.fillRect(20 + x, centre - a, 3, a * 2);
      }
      gE.fillStyle = 'rgba(255,255,255,0.22)';
      gE.fillRect(14, centre - 0.5, w - 28, 1);
    });

    var tete = 20 + ((t * 26) % (w - 44));
    gE.fillStyle = '#ffffff';
    gE.fillRect(tete, 56, 1.6, 232);
    texEcran.needsUpdate = true;
  }

  var laptop = new THREE.Group();
  laptop.position.set(-0.75, 0, 0.35);
  laptop.rotation.y = 0.42;

  var socle = new THREE.Mesh(new THREE.BoxGeometry(2.3, 0.1, 1.6), mAlu);
  socle.position.y = 0.05;
  socle.castShadow = true; socle.receiveShadow = true;
  laptop.add(socle);

  var clavier = new THREE.Mesh(new THREE.BoxGeometry(1.95, 0.012, 0.72), mSombre);
  clavier.position.set(0, 0.106, -0.16);
  laptop.add(clavier);
  var pave = new THREE.Mesh(new THREE.BoxGeometry(0.66, 0.012, 0.42), new THREE.MeshStandardMaterial({ color: 0xa9acb3, roughness: 0.35, metalness: 0.7 }));
  pave.position.set(0, 0.106, 0.42);
  laptop.add(pave);

  var capot = new THREE.Group();
  capot.position.set(0, 0.1, -0.78);
  capot.rotation.x = -0.28;
  var dos = new THREE.Mesh(new THREE.BoxGeometry(2.3, 1.5, 0.07), mAlu);
  dos.position.y = 0.75;
  dos.castShadow = true;
  capot.add(dos);
  var dalle = new THREE.Mesh(
    new THREE.PlaneGeometry(2.06, 1.29),
    new THREE.MeshBasicMaterial({ map: texEcran })
  );
  dalle.position.set(0, 0.75, 0.038);
  capot.add(dalle);
  laptop.add(capot);
  plateau.add(laptop);

  var micro = new THREE.Group();
  micro.position.set(1.45, 0, 0.75);
  micro.rotation.y = -0.35;

  var pied = new THREE.Mesh(new THREE.CylinderGeometry(0.36, 0.4, 0.07, 40), mSombre);
  pied.position.y = 0.035;
  pied.castShadow = true; pied.receiveShadow = true;
  micro.add(pied);

  var mat = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.95, 20), mAlu);
  mat.position.y = 0.5;
  mat.castShadow = true;
  micro.add(mat);
  ...
  tete.rotation.z = 0.38;

  var pince = new THREE.Mesh(new THREE.TorusGeometry(0.2, 0.035, 10, 28), mCaoutchouc);
  pince.rotation.y = Math.PI / 2;
  tete.add(pince);

  var corps = new THREE.Mesh(new THREE.CylinderGeometry(0.17, 0.17, 0.52, 32), mSombre);
  corps.position.y = 0.1;
  corps.castShadow = true;
  tete.add(corps);

  var grille = new THREE.Mesh(new THREE.SphereGeometry(0.175, 28, 20), mGrille);
  grille.position.y = 0.4; grille.scale.y = 1.25;
  grille.castShadow = true;
  tete.add(grille);

  var bague = new THREE.Mesh(new THREE.TorusGeometry(0.175, 0.02, 10, 32), mAccent);
  bague.rotation.x = Math.PI / 2;
  bague.position.y = 0.26;
  tete.add(bague);

  var temoin = new THREE.Mesh(
    new THREE.SphereGeometry(0.035, 14, 12),
    new THREE.MeshStandardMaterial({ color: 0xff4d4d, emissive: 0xff2222, emissiveIntensity: 1, roughness: 0.4 })
  );
  temoin.position.set(0, -0.08, 0.165);
  tete.add(temoin);

  var culot = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.11, 0.12, 20), mSombre);
  culot.position.y = -0.2;
  tete.add(culot);

  micro.add(tete);
  plateau.add(micro);

  var chemin = new THREE.CatmullRomCurve3([
    new THREE.Vector3(1.42, 0.82, 0.82),
    new THREE.Vector3(1.52, 0.42, 1.05),
    new THREE.Vector3(1.34, 0.055, 1.28),
    new THREE.Vector3(0.72, 0.055, 1.15),
    new THREE.Vector3(0.35, 0.09, 0.78),
    new THREE.Vector3(0.30, 0.10, 0.42)
  ]);
  var cable = new THREE.Mesh(new THREE.TubeGeometry(chemin, 80, 0.028, 10, false), mCaoutchouc);
  cable.castShadow = true;
  plateau.add(cable);

  var prise = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.06, 0.13), new THREE.MeshStandardMaterial({ color: 0x6d7076, roughness: 0.35, metalness: 0.8 }));
  prise.position.set(0.30, 0.10, 0.36);
  plateau.add(prise);

  scene.add(new THREE.AmbientLight(0xffffff, 0.5));
  var key = new THREE.DirectionalLight(0xfff3e2, 1.1);
  ...
  key.shadow.bias = -0.0012;
  scene.add(key);
  var fill = new THREE.DirectionalLight(0x9fb4d8, 0.42);
  fill.position.set(-5, 2, 3.5);
  scene.add(fill);
  var lueur = new THREE.PointLight(ACCENT, 0.55, 4.5);
  lueur.position.set(-0.7, 1.0, 0.9);
  scene.add(lueur);
  ...
  function cadre() {
    ...
    camera.aspect = aspect;
    var demiTan = Math.tan(Math.PI * 32 / 360);
    var z = 2.75 / (demiTan * aspect);
    camera.position.set(0, 2.25, Math.min(14, Math.max(5.4, z)));
  ...
  var lent = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  var t = 0, px = 0, cx = 0, visible = true, boucle = null, image = 0;
  ...
  function dessine() {
    if (image % 2 === 0) dessineEcran(t);
    tete.rotation.z = 0.38 + Math.sin(t * 0.9) * 0.035;
    micro.rotation.y = -0.35 + Math.sin(t * 0.7) * 0.03;
    temoin.material.emissiveIntensity = 0.55 + Math.abs(Math.sin(t * 1.6)) * 0.9;
    cx += (px - cx) * 0.05;
    plateau.rotation.y = cx * 0.22;
    renderer.render(scene, camera);
  }
  ...
  window.addEventListener('resize', function () {
    ...
    renderer.setSize(W, H);
    cadre();
    if (lent || !visible) { dessineEcran(t); renderer.render(scene, camera); }
  });
